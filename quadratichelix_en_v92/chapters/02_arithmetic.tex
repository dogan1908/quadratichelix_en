...
\begin{remark}[Connection to the OEIS: A164576]
	...
	no $(-1)^j$ correction term.
The sequence itself is thus not new; what is new in the present
work is its geometric interpretation.
\end{remark}
...
\begin{theorem}\label{satz:kongruenz}
...
\end{tabular}
\end{center}
The final column includes the constant term $+1$ of the numerator.
The sum $2n^2 + 3n + 1$ is divisible by $6$ precisely in the rows
$n \equiv 1$ and $n \equiv 5$, which proves the claim.
\end{proof}
...
\begin{definition}[Minor and Major Steps]\label{def:minor_major}
...
into the context of simple linear and quadratic sequences. The
...
(Theorem~\ref{satz:kongruenz}), the generating function thus provides an
