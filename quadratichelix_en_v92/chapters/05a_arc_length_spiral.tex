\chapter{Arc Length of the Plane Archimedean Spiral: Two Methods}
...
approaches and the results are compared numerically. This serves both for
mutual verification and for the conceptual clarification of how the
$\theta$-parametrisation and the $p$-parametrisation are related. Throughout
the numerical tables the decimal point is used consistently.

The intervals under consideration are the minor steps between the
...
For a curve in polar coordinates $r = r(\theta)$ the arc-length
element reads
\[
...
The arc-length integral between two angles $\theta_1$ and $\theta_2$ is therefore:
\[
L_A(\theta_1, \theta_2)
...
\]
we obtain the antiderivative $F(\theta) = \frac{a}{2}\bigl[\theta\sqrt{\theta^2+1}
+ \operatorname{arcsinh}(\theta)\bigr]$ and hence the exact, closed-form arc length:

\begin{theorem}[Arc length of the Archimedean spiral, closed form]
...
\subsection{Assignment of Angle Values to the Indices}

The angle values $\theta_n$ at the integer points of the helix are
\[
...
\begin{remark}[Constant angular spacing]
...
nevertheless, the arc length grows monotonically, since for the same
$\Delta\theta$ the arc on a larger radius is longer.
\end{remark}
...
\begin{table}[H]
\centering
\caption{Arc length by Method~A: closed-form formula
...
\begin{remark}[Growth rate of the arc length]
The arc lengths grow approximately proportionally to the mean $\theta$-value.
For large $\theta$ the linear term in $\sqrt{\theta^2+1} \approx \theta$
dominates, so that
\[
...
\]
With $\Delta\theta = \frac{2\pi}{3}$ and increasing
$\bar\theta = \frac{\theta_1+\theta_2}{2}$, a linear growth
in $\bar\theta$ results. The difference between two consecutive
arc lengths is therefore approximately
$a \cdot \frac{2\pi}{3} \cdot 2\pi = a \cdot \frac{4\pi^2}{3} = 8$ per
period of the indices (from $n$ to $n+6$). Numerically:
$25.032 - 17.048 = 7.985$, $33.025 - 25.032 = 7.993$
etc.\ --- this convergence confirms the asymptotics.
\end{remark}
...
The points in the $p$-parametrisation correspond to the function values
$p = k(n)$:
...
\begin{remark}[Growth of the $\Delta p$ values]
...
The function values $p$ grow quadratically in $n$, while $\Delta p$
grows only linearly. This implies that for large $p$ the quotient
$\Delta p / p$ tends to zero --- the integrand term $\frac{3}{p}$ then
becomes negligible compared with $\pi^2$, and the integral approaches the
...
\begin{table}[H]
\centering
\caption{Arc length by Method~B: numerical integral
...
\caption{Comparison of arc lengths: Method~A (closed-form $\theta$-formula)
  ...
  is positive and decreases monotonically through the computed range towards zero.}
...
The relative deviation $\delta = (L_B - L_A)/L_A$ is positive in all computed intervals and converges to zero:
\[
...
\begin{remark}[Explanation of the small positive deviation]
The deviation stems from the fact that the $p$-parametrisation is only
\emph{asymptotically} arc-length preserving: the inversion $p = L(\theta)$
holds asymptotically for large $\theta$, not for small ones. For the first interval
($p_1 = 11$, $p_2 = 20$) the correction is still measurable; for
$p \gtrsim 100$ it is negligible for all practical purposes.
...
The term $\frac{3}{\pi^2 p}$ produces the positive residual deviation; it is
approximately $0.028$ for $p_1 = 11$, but only about $0.0005$ for $p_1 = 581$.
\end{remark}

\subsection{Why the Equidistant Parametrisation Works}

The central conceptual statement of this comparison is that the
integer values $p = k(n)$ of the quadratic sequence already constitute
a very good approximation to the arc-length values of the Archimedean
spiral. Specifically: the step $\Delta p = p_2 - p_1 = \Delta_{\mathrm{Minor}}(m)$
is numerically almost equal to the arc length $L_A$ of the corresponding
spiral arc:

\begin{table}[H]
\centering
\caption{Comparison of $\Delta p$ (arithmetic step) vs.\ $L_A$ (arc length).
...
This table is the numerical heart of the entire construction: it shows
that the arithmetic steps $\Delta p = 8m+1$ of the minor difference sequence
are not only integers but also increasingly accurate approximations of the
arc lengths of the corresponding spiral arcs. For $m \to \infty$ one has
...
Put differently: the construction succeeds because the
function values $k(n)$ are not only integers that enforce the correct spiral
constant, but simultaneously approximate arc-length parameters of the
