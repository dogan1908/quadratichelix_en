\chapter{Arc Length Comparison: Parabola vs.\ Helix}\label{sec:beispiele}

\section{Principle of the Comparison}

The quadratic curve $k(n) = \frac{1}{6}(2n^2 + 3n + 1)$ was geometrically
wound onto a cone, forming the helix $\gamma(\theta)$.
...
\textbf{Arc length of the helix} with direct $\theta$-parametrization:
\[
...
The arc length thus follows as:
\[
...
the remaining results are summarized in
Table~\ref{tab:bogenlaengenvergleich}.
...
\begin{remark}
The helix is \emph{longer} than the parabola, since the
spiral motion in the $(x,y)$-plane (especially at small
$\theta$) generates additional path length.
\end{remark}
...
\caption{Arc length comparison: Parabola vs.\ Helix
  (exact $\theta$-parametrization).}
\label{tab:bogenlaengenvergleich}
...
$\theta = \pi Q/12$, hence $d\theta = (\pi/12)\,dQ$, we obtain:

\begin{theorem}[$Q$-Parametrization of the Arc Lengths]\label{satz:q-bogenlaenge}
The arc length elements of the parabola $k(n)$ and the conical helix can be
expressed uniformly as
\begin{equation}\label{eq:ds-einheitlich}
...
$\theta = \pi Q/12$, we have $d\theta = (\pi/12)\,dQ$. The arc length element is
$ds = \sqrt{a^2(1+\theta^2) + c_1^2}\,d\theta$. Substituting
$\theta = \pi Q/12$:
\begin{align*}
  a^2(1+\theta^2) + c_1^2
...
$(x,y)$-plane. For $Q \to \infty$, the term $Q^2$ dominates in both cases,
and the ratio of the arc length elements converges:
\[
...
Table~\ref{tab:bogenlaengenvergleich} analytically, not merely
numerically.
\end{remark}
...
\label{prop:konvergenz}
The helix arc length is consistently \emph{longer} than the arc length of the
corresponding parabolic segments, with the relative difference
...
\]
This follows directly from the unified $Q$-representation
(Theorem~\ref{satz:q-bogenlaenge}), since $C_{\mathrm{Helix}}/Q^2 \to 0$
and $C_{\mathrm{Parabola}}/Q^2 \to 0$ as $Q \to \infty$.
\end{proposition}

\begin{corollary}[Practical Accuracy]
For all primes $p > 11$ (corresponding to $n \ge 11$), the deviation
lies below $0{,}3\,\%$. For applications in the visualization of prime number distributions,
