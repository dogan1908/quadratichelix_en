...
In this chapter we investigate the number-theoretic properties of the sequence~\eqref{eq:k_main} and derive the structure of its differences.

\section{Derivation via Means of Sums of Squares}

The sequence $k(n)$ arises naturally from the classical summation formula for square numbers. For the sum of the first $n$ squares, the well-known identity
...
\begin{bemerkung}
The sequence $k(n)$ thus gives the \emph{average value} of the first $n$ square numbers. It is the parabolic interpolation form of the arithmetic mean. This interpretation is important for understanding the subsequent geometric construction, as it establishes the connection between discrete summation and continuous geometry.
\end{bemerkung}
...
The boldface values indicate the integer means, which occur precisely for $n \equiv 1, 5 \pmod{6}$.

\begin{bemerkung}[Connection to OEIS: A164576]
	...
	as \emph{``Integer averages of the set of the first positive
		squares up to some $n^2$''} -- precisely the sequence $k(n)$ whose discovery
	...
	odd $j$ (corresponding to $n \equiv 1 \pmod{6}$) and even $j$
	(corresponding to $n \equiv 5 \pmod{6}$), it alternately takes the values
	$+1$ and $-1$.
	
	The connection to $k(n)$ is established by substituting
	$A007310(j)$ -- the sequence of positive integers coprime
	to~$6$~\cite{OEIS}:
	\[
	a(j) = k\!\bigl(A007310(j)\bigr),
	\]
	where the two branches yield exactly:
	\[
	k(A007310(j)) =
	\begin{cases}
	...
	is simpler and more direct: it operates with the actual
...
\begin{satz}
The sequence $k(n)$ assumes integer values if and only if
\begin{equation}
n \equiv 1, 5 \pmod{6}.
\end{equation}
\end{satz}
...
\begin{satz}[Prime Number Property]\label{satz:primzahlen}
All primes $p > 3$ belong to the classes $n \equiv 1, 5 \pmod{6}$.
\end{satz}

\begin{proof}
Every prime $p > 3$ is coprime to~$6$, since it is divisible by neither~$2$ nor~$3$. The residue classes coprime to~$6$ are precisely
...
\begin{figure}[htbp]
  ...
  \caption{The quadratic curve $k(n) = \tfrac{1}{6}(2n^2+3n+1)$ as a continuous
    parabola with its numerical table of values. The orange dots mark the
    integer values at $n \equiv 1,5 \pmod{6}$; the violet dots
    correspond to the non-integer intermediate values. The growth of the sequence
    is already clearly visible at the first values $(1,\,1),\,(5,\,11),\,(7,\,20)$.}
...
We consider the sequence of quadratic mean values
...
The closed forms of these series are well known:
...
where all series converge for $|x|<1$. Substitution yields
...
\begin{bemerkung}
The rational structure of $K(x)$ reflects the fact that $k(n)$ is described by a polynomial of degree two in $n$. From the representation of $K(x)$ one can, for instance, obtain a linear recurrence for $k(n)$ by considering the common denominator $(1-x)^{3}$ and comparing coefficients of $x^{n}$. 

At the same time, $K(x)$ arises as a linear combination of the generating functions of $n$, $n^{2}$, and the constant sequence $1$. Thus $k(n)$ fits naturally into the context of simple linear and quadratic sequences. The generating function provides an alternative, analytic perspective on the same structure that is already visible discretely in the congruence-theoretic considerations (Theorem~\ref{satz:primzahlen}).
\end{bemerkung}

\subsection{Linear Recurrence from the Generating Function}

From the rational representation of $K(x)$, a linear recurrence for the sequence $k(n)$ can be obtained immediately. To this end we write
...
Expanding the product and comparing the coefficients of $x^{n}$ for sufficiently large $n$, one obtains
...
\begin{bemerkung}
The recurrence reflects the fact that $k(n)$ is given by a polynomial of degree $d = 2$ in $n$: a sequence described by a polynomial of degree $d$ always satisfies a linear recurrence of constant order $d+1$ with constant coefficients. In the present case $d=2$, and the order of the recurrence is $3$.
...
and the congruence-theoretic condition for integer values (Section~\ref{sec:arithmetik}), the generating function thus provides an alternative, analytic perspective on the same structure. It connects the discrete summation formula, the quadratic form of $k(n)$, and the arithmetic patterns of the sequence within a unified framework.
\end{bemerkung}
