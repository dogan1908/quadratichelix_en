\chapter{The Conical Helix}\label{sec:helix}

\section{The Helix as a Trace on the Cone Surface}

When the original spiral is coupled with the linear height $z=n$,
a space curve emerges that winds along the cone surface.
This curve is a \emph{conical helix}: It combines the radial structure of the
spiral with the vertical structure of the sequence and constitutes the three-dimensional
shape of the construction.

\section{Complete Helix Parametrization}

\begin{theorem}[Conical Helix]
\begin{equation}
\boxed{
\gamma(\theta) =
\begin{pmatrix}
...
\begin{figure}[htbp]
  ...
  \caption{The conical helix $\gamma(\theta)$ in perspective view along
    the $z$-axis. The red curve winds upward with increasing radius. The
    green points mark the integer height values
    $z = n$ with $n \equiv 5 \pmod{6}$, the orange points the values with
    ...
    $\Delta\theta_{\text{minor}} = 2\pi/3$ between consecutive
    integer points.}
  \label{fig:helix_integer}
\end{figure}
...
\right),
\]
which consists of a linear height function and a radially expanding circular component.
To determine the arc length of this curve, we first require the
speed $\|\gamma'(t)\|$.

\subsection*{1. Derivatives of the Components}

We differentiate each component separately. For the first two
components, we apply the product rule, since each involves a product of $t$ with a
...
The height function is linear, hence:
\[
...
Substituting the previously computed derivatives:
\[
\|\gamma'(t)\|^{2}
=
...
The two quadratic terms can be simplified by expansion.
...
Since $\cos^{2}t + \sin^{2}t = 1$ holds universally, it follows that:
\[
(\cos t - t\sin t)^{2} + (\sin t + t\cos t)^{2}
= 1 + t^{2}.
\]
...
The arc length between two parameters $t_1$ and $t_2$ is therefore:
...
%------------
\section{Arc Length of the Helix in the $p$-Parametrization}

It is important to emphasize that the formula derived in Chapter~\ref{sec:bogenlaenge},
\begin{equation}
L_{\mathrm{Spiral}} = \int_{p_1}^{p_2}
\frac{1}{\pi}\sqrt{\frac{3}{p} + \pi^2}\, dp,
\end{equation}
computes the arc length of the \emph{two-dimensional spiral} in the $p$-parametrization.
The arc length of the three-dimensional helix (with the above
$\theta$-parametrization) is a \emph{different quantity}, since it includes the additional
vertical component $z'(\theta) = 3/\pi$. The numerical differences
observed in Chapter~\ref{sec:beispiele} between the two arc lengths arise
because they are arc lengths of \emph{different curves}
(the planar spiral on one hand, the space curve on the other), not from an error
in the parametrization.

\section{Arc Length Between Integer Values of $n$}
...
\midrule
$1 \to 5$ & 4 & Major & $\frac{4\pi}{3}$ & 11.155199 \\
$5 \to 7$ & 2 & Minor & $\frac{2\pi}{3}$ & 9.309137 \\
$7 \to 11$ & 4 & Major & $\frac{4\pi}{3}$ & 26.434772 \\
\rowcolor{yellow!30}
\textbf{11} $\to$ \textbf{13} & \textbf{2} & \textbf{Minor} & $\frac{2\pi}{3}$ & \textbf{17.164862} \\
$13 \to 17$ & 4 & Major & $\frac{4\pi}{3}$ & 42.268251 \\
$17 \to 19$ & 2 & Minor & $\frac{2\pi}{3}$ & 25.112276 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht!]
\centering
\caption{Parameter values $\theta_n$, reduced angles, and radii of the conical helix.
  All radii were computed exactly as $r_n = (6/\pi^2)\,\theta_n$ and
...
\section{Complete Revolution of the Helix}

A complete revolution about the $z$-axis corresponds to $\Delta\theta = 2\pi$.
\begin{equation}
L_{360^\circ}
= \int_{0}^{2\pi}
\sqrt{
...
Thus the helix rises by exactly 6 units per revolution.
...
\begin{remark}[Planar Spiral Curvature as an Approximation]
The curvature of a space curve $\gamma(t)$ is defined as
$\kappa = |\gamma' \times \gamma''| \, / \, |\gamma'|^3$~\cite{doCarmo1976,Struik1988} and depends, in the case of the
conical helix, on the height component $z'(\theta) = 3/\pi$. As
a tractable approximation, the formula for the planar Archimedean spiral
can be employed, which for large $\theta$ converges rapidly to the exact
3D curvature (deviation below 2\,\%):
\end{remark}
...
\begin{theorem}[Exact 3D Curvature and Torsion]\label{satz:3d-kruemmung}
For the conical helix $\gamma(\theta) = \bigl(a\theta\cos\theta,\;
a\theta\sin\theta,\; c_0 + c_1\theta\bigr)$ with $a = 6/\pi^2$ and
$c_1 = 3/\pi$, the following exact formulas hold:
\begin{align}
...
\begin{proof}
The derivatives of the parametrization are
\begin{align*}
...
\[
  ...
  = \bigl(-ac_1(2\cos\theta - \theta\sin\theta),\;
          ...
          a^2(\theta^2+2)\bigr)
\]
with magnitude $|\gamma' \times \gamma''|^2 = a^2\bigl[c_1^2(\theta^2+4) +
a^2(\theta^2+2)^2\bigr]$ and $|\gamma'|^2 = a^2(1+\theta^2) + c_1^2$.
The curvature follows from $\kappa = |\gamma' \times \gamma''|/|\gamma'|^3$.

For the torsion, the scalar triple product yields
$(\gamma' \times \gamma'') \cdot \gamma''' = a^2 c_1(\theta^2+6)$,
from which $\tau = (\gamma' \times \gamma'') \cdot \gamma''' \,/\,
|\gamma' \times \gamma''|^2$ follows.
\end{proof}

\begin{remark}
For $\theta \to \infty$, the asymptotic behavior is
$\kappa_{\mathrm{3D}}(\theta) \sim 1/(a\theta)$ and
$\tau(\theta) \sim c_1/a^2 = \pi^3/12$, i.e., the torsion
converges to a nonzero constant --
in contrast to the planar spiral, whose torsion vanishes identically.
\end{remark}
...
Thus all $n \equiv 1,5 \pmod{6}$ lie as discrete points on the
helix. The complete analysis of these integer points -- including
the prime number corollary -- follows in Chapter~\ref{ch:primes}.
