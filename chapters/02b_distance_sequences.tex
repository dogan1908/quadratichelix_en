...
$(n_i,\, k(n_i))$ on the curve $k(n) = \frac{1}{6}(2n^2+3n+1)$ can be
completely described by closed-form expressions. Since $\Delta n$ alternates
between $2$ (twin zone) and $4$ (gap zone), two subsequences arise
with explicit expressions in the period index~$m$.

%─────────────────────────────────────────────────────────────────────────────
\subsection{Twin Zone Distance Sequence (\texorpdfstring{$\Delta n = 2$}{Δn=2})}
%─────────────────────────────────────────────────────────────────────────────

\begin{satz}[Closed-form expression for twin distances]
...
\begin{satz}[Closed-form expression for gap distances]
...
When both zones are merged in the natural order of the indices
and labelled with the running index $x = 1, 2, 3, \ldots$,
the following unified representation in $x$ is obtained:

\begin{satz}[Distance sequence in global index $x$]
\label{satz:distanz_gesamtindex}
With the global index $x = 1, 2, 3, \ldots$ (gap for odd, twin for
even $x$):
\begin{equation}
  d^2(x) =
  \begin{cases}
...
\begin{bemerkung}[Precursor point and zero]
	...
	as a sequence term -- not because of the residue class, but because of the
	restriction to positive indices.
	
	As a geometric precursor point, $(-1,\, 0)$ nevertheless provides a
	natural interpretation: the distance to the first genuine sequence term
	$(1,\, 1)$ is
	\[
	d = \sqrt{(1-(-1))^2 + (1-0)^2} = \sqrt{5},
	\]
	a value that appears in the sequence as a natural starting value before $k(1) = 1$.
\end{bemerkung}

\begin{bemerkung}[Growth rate]
