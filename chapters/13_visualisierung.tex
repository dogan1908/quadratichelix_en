...
\midrule
   10 & 5.7588 &  3.6608 & 1.01438 \\
  100 & 18.138 & 11.531  & 1.00151 \\
 1000 & 57.329 & 36.443  & 1.00015 \\
10000 & 181.28 & 115.24  & 1.00002 \\
\bottomrule
\end{tabular}
\end{table}
...
reveals a deep shared character of both structures.
...
The sequence $k(n)$ is, in a sense, \emph{the natural indexing}
...
A & $\dfrac{\pi\sqrt{k}}{\sqrt{3}}$
  & $\sqrt{3} \approx 1.732$
  & Project standard (\texttt{Power\_equidistant\_Spiral\_v6}) \\[8pt]
B & $\dfrac{2\pi\sqrt{k}}{h}$
  & $2\sqrt{3} \approx 3.464$
  & \texttt{equidistant\_Spiral\_v5} \\[8pt]
...
\]
The project uses \textbf{Form~A} with the natural scaling factor
...
\]
from which a spiral angle $\theta_{p(n)} \approx \tfrac{\pi}{\sqrt{a}}\cdot\sqrt{a}\,n$
is obtained. The approximation method requires only knowledge of the
...
that is irrational for general $k$. The remarkable fact is:
...
The fundamental identity~\eqref{eq:fundamental} suggests treating
the expression under the square root as an independent quantity:
...
The exact formula~\eqref{eq:n_von_k} possesses a natural
